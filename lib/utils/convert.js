...
function editorStateToHtml(editorState) {
  ...
  var removeMeTag = ':removeMe:';
  var convertedHTMLWithRemoveTags = convertedHTML.replace(/href="http/g, 'href="http' + removeMeTag);
  var convertedHTMLLinkify = convertedHTMLWithRemoveTags;
  var linkifyMatch = linkify.match(convertedHTMLWithRemoveTags);
  if (linkifyMatch !== null) {
    convertedHTMLLinkify = linkifyMatch.filter(function (match) {
      ...
      var urlWithTag = match.url.slice(0, 8) + ':removeMe:' + match.url.slice(8);
